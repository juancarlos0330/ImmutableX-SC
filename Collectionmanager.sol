...
contract CollectionManager {
    ...
    address public treasury = 0x076F605BF8d2bD03c991cECcc2B4a1d5bC50BaaB; // Treasury Account
    ...
    // Import NFT which minted in our platform
    function importnft(address _nftaddress, string memory _CollectionName) public {
        collectionnameexists[_CollectionName] = true;
        collectionnames.push(_CollectionName);
        currentcollections++;
        nftcollectionsbyindex[currentcollections] = NFTCollection(_nftaddress);
        nftcollectionsbyname[_CollectionName] = NFTCollection(_nftaddress);
    }
    ...
    function publicmint(string memory _collectionName, string memory _tokenURI) public payable {
        ...
        for (uint256 i=0; i < collaboratorsbycollectioname[_collectionName].length; i++ ) {
            address colla = collaboratorsbycollectioname[_collectionName][i];
            uint256 funds =  msg.value * royaltyvaluepercollaborators[colla] / 100;
